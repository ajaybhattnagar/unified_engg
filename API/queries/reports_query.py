reports_query = {
    ...
    "SUB_REQUEST_FORM": """SELECT PARCELS.STATE, PARCELS.MUNICIPALITY, PARCELS.UNIQUE_ID, PARCELS.CERTIFICATE,
                            PARCELS.LEGAL_BLOCK, PARCELS.LEGAL_LOT_NUMBER, PARCELS.QUALIFIER, PARCELS.APN, PARCELS.STATUS
                            FROM PARCELS
                            WHERE PARCELS.UNIQUE_ID IS NOT NULL""",
    

    "LIEN_DETAILS_WEEKLY_REPORT_HEADER": """SELECT PARCELS.UNIQUE_ID, PARCELS.STATUS, PARCELS.STATE, PARCELS.MUNICIPALITY, PARCELS.COUNTY, PARCELS.COUNTY_LAND_USE_DESC,
    ...
    "NEW_PENDING_REDEMPTION_NOTICE_TO_WSFS": """SELECT PARCELS.COUNTY, PARCELS.STATE, PARCELS.UNIQUE_ID, PARCELS.ORIGINAL_LIEN_EFFECTIVE_DATE, PARCELS.LOCATION_FULL_STREET_ADDRESS, PARCELS.LOCATION_CITY, PARCELS.LOCATION_ZIP,
                                                PARCELS.LEGAL_BLOCK, PARCELS.LEGAL_LOT_NUMBER, PARCELS.QUALIFIER, PARCELS.ORIGINAL_LIEN_AMOUNT, PARCELS.STATUS, PARCELS.CERTIFICATE
                                                FROM PARCELS
                                                WHERE PARCELS.UNIQUE_ID IS NOT NULL""",

    "WSFS_REDEMPTION_NOTIFICATION": """SELECT PARCELS.STATE, PARCELS.COUNTY, PARCELS.PARCEL_ID, PARCELS.CERTIFICATE, PARCELS.UNIQUE_ID, PARCELS.STATUS,
                                        RED.DATE_REDEEMED
                                        FROM PARCELS
                                        LEFT JOIN (SELECT UNIQUE_ID, MIN(DATE_REDEEMED) 'DATE_REDEEMED', SUM(CHECK_AMOUNT) 'CHECK_AMOUNT', MIN(CHECK_RECEIVED) 'CHECK_RECEIVED'
                                        FROM REDEEM 
