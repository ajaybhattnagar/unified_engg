reports_query = {
    ...
    "FEE_DETAIL_REPORT": """
        ...
        CASE 
            ...
            WHEN FEES.CATEGORY = 11 THEN 'Redemption Variance'
            WHEN FEES.CATEGORY = 12 THEN 'Overage'
            WHEN FEES.CATEGORY = 13 THEN 'TDA Rollup'
        ...
        ELSE 0 END AS 'AMOUNT',
        CASE 
            WHEN FEES.CATEGORY BETWEEN 2 AND 12 THEN 0
            WHEN FEES.CATEGORY < 3 THEN FEES.AMOUNT
            WHEN FEES.CATEGORY = 13 THEN FEES.AMOUNT
        ELSE 0 END AS 'FEES',
        FEES.INTEREST, FEES.INTEREST_ACC_INTERVAL, CONVERT(FEES.EFFECTIVE_DATE, DATE) 'EFFECTIVE_DATE',
        CASE
            WHEN CONVERT(FEES.EFFECTIVE_END_DATE, DATE) > '1994-10-21' THEN CONVERT(FEES.EFFECTIVE_END_DATE, DATE)
        ...
        FROM FEES
        LEFT JOIN PARCELS ON PARCELS.UNIQUE_ID = FEES.UNIQUE_ID
        WHERE FEES.IS_ACTIVE = '1' AND PARCELS.UNIQUE_ID IS NOT NULL -- AND PARCELS.UNIQUE_ID = '66e1eb0e'""",
    ...
    "LIEN_DETAILS_WEEKLY_REPORT_HEADER": """SELECT PARCELS.UNIQUE_ID, 
                                            ...
                                            TOTAL_CHECK_AMT.AMOUNT 'TOTAL_CHECK_AMT'
    ...
    "NEW_PENDING_REDEMPTION_NOTICE_TO_WSFS": """SELECT 
                                                ...
                                                PARCELS.LEGAL_BLOCK 'LEGAL BLOCK', PARCELS.LEGAL_LOT_NUMBER 'LEGAL LOT NUMBER', PARCELS.QUALIFIER, PARCELS.ORIGINAL_LIEN_AMOUNT 'BEGINNING BALANCE',
                                                CASE 
                                                WHEN PARCELS.STATUS = 1 THEN 'ACTIVE'
                                                WHEN PARCELS.STATUS = 2 THEN 'PENDING'
                                                WHEN PARCELS.STATUS = 3 THEN 'PENDING REDEMPTION'
                                                ...
                                                ELSE 'ERROR' END AS 'STATUS',
                                                PARCELS.CERTIFICATE,
                                                IFNULL(PAY.PAYMENT, 0) AS 'PAYMENT', TDA_ROLL.TDA 'TDA ROLL UPS'

                                                FROM PARCELS
                                                LEFT JOIN (SELECT UNIQUE_ID, SUM(CHECK_AMOUNT) AS 'PAYMENT' FROM REDEEM GROUP BY UNIQUE_ID) PAY ON PAY.UNIQUE_ID = PARCELS.UNIQUE_ID
                                                LEFT JOIN (SELECT UNIQUE_ID, SUM(AMOUNT) 'TDA' FROM FEES  WHERE CATEGORY = 13 GROUP BY UNIQUE_ID) TDA_ROLL ON TDA_ROLL.UNIQUE_ID = PARCELS.UNIQUE_ID

                                                WHERE PARCELS.UNIQUE_ID IS NOT NULL -- and PARCELS.UNIQUE_ID in ('d3371af2', '501274b3')""",
    ...
    "WSFS_LTVL_STATUS" : """SELECT PARCELS.COUNTY, PARCELS.STATE, PARCELS.MUNICIPALITY, PARCELS.UNIQUE_ID 'REFERENCE ID', PARCELS.ORIGINAL_LIEN_EFFECTIVE_DATE 'BEGINNING BALANCE EFFECTIVE DATE',
                                ...
                                PARCELS.ORIGINAL_LIEN_AMOUNT 'BEGINNING BALANCE', NULL 'TOTAL REDEEMABLE', PARCELS.STATUS, TDA_ROLL.TDA 'TDA ROLL UP',
                                CASE 
                                    WHEN PARCELS.STATUS = 1 THEN 'ACTIVE'
                                    WHEN PARCELS.STATUS = 2 THEN 'PENDING'
                                    WHEN PARCELS.STATUS = 3 THEN 'PENDING REDEMPTION'
                                ...
                                SUBS.AMOUNT 'SUB 1 AMOUNT', PARCELS.PREMIUM_AMOUNT 'PREMIUMS', PARCELS.CREATE_DATE 'ACTIVE', 
                                ...
                                REDE.CHECK_RECEIVED 'REDEEMED', TDA.TDA_DT 'TDA' 
                                FROM PARCELS 
                                ...
                                LEFT JOIN (SELECT UNIQUE_ID, MIN(DONE_AT) 'TDA_DT'
                                            FROM AUDIT 
                                            WHERE NEW_VALUE = 11 AND COLUMN_NAME = 'PARCEL_STATUS' 
                                            GROUP BY UNIQUE_ID) TDA ON TDA.UNIQUE_ID = PARCELS.UNIQUE_ID
                                LEFT JOIN (SELECT UNIQUE_ID, SUM(AMOUNT) 'TDA' FROM FEES  WHERE CATEGORY = 13 GROUP BY UNIQUE_ID) TDA_ROLL ON TDA_ROLL.UNIQUE_ID = PARCELS.UNIQUE_ID
                                            
                                -- WHERE PARCELS.UNIQUE_ID = '726ae025'
                            """
