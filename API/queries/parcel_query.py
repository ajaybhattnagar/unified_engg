...
"SEARCH_PARCEL": """SELECT PARCELS.*,
                    RED.FIRST_CHECK_DATE
                    FROM PARCELS 
                    LEFT JOIN (SELECT UNIQUE_ID, MIN(CHECK_RECEIVED) 'FIRST_CHECK_DATE'
                                FROM REDEEM GROUP BY UNIQUE_ID) RED ON RED.UNIQUE_ID = PARCELS.UNIQUE_ID
                    WHERE TSRID LIKE '{TEXT}' OR STATE LIKE '{TEXT}' OR MUNICIPALITY LIKE '{TEXT}' 
                    OR LOCATION_FULL_STREET_ADDRESS LIKE '{TEXT}' OR PARCELS.UNIQUE_ID LIKE '{TEXT}' OR PARCELS.PARCEL_ID LIKE '{TEXT}' OR PARCELS.CERTIFICATE LIKE '{TEXT}'
                    """,
