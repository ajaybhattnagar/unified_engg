...
"EMPLOYEE_LAST_30_LABOR_TICKETS": """SELECT DISTINCT TOP 30 WORKORDER_BASE_ID, WORKORDER_LOT_ID, WORKORDER_SPLIT_ID, WORKORDER_SUB_ID, ISNULL(CAST(WO.USER_1 AS VARCHAR), '') AS [WO_DESC],
                                        OPERATION_SEQ_NO, RESOURCE_ID
                                        ...
                                        WHERE LAB.TYPE = 'R' AND WO.STATUS IN ('R', 'F', 'U') AND CAST(LAB.CREATE_DATE AS DATE) >= CAST((GETDATE() - 7) AS DATE) AND EMPLOYEE_ID = '{EMP_ID}'
                                        ORDER BY WORKORDER_BASE_ID DESC""",
...
"EMPLOYEE_KPIS": """
                    SELECT 
                    ISNULL((SELECT SUM(HOURS_WORKED) FROM UNI_LABOR_TICKET WHERE EMPLOYEE_ID = '{EMP_ID}' AND DATEPART(ISO_WEEK, TRANSACTION_DATE) = DATEPART(ISO_WEEK, GETDATE()) GROUP BY EMPLOYEE_ID),0) AS [TOTAL_WEEK_HRS], 
                    ISNULL((SELECT SUM(HOURS_WORKED) FROM UNI_LABOR_TICKET WHERE EMPLOYEE_ID = '{EMP_ID}' AND  CONVERT(DATE, TRANSACTION_DATE) =  CONVERT(DATE, GETDATE()) GROUP BY EMPLOYEE_ID),0) AS [TOTAL_TODAY_HRS]
                                        """,
...
"DUPLICATE_LABOUR_TICKET": """INSERT INTO UNI_LABOR_TICKET (WORKORDER_TYPE, WORKORDER_BASE_ID, WORKORDER_LOT_ID, WORKORDER_SPLIT_ID, WORKORDER_SUB_ID, OPERATION_SEQ_NO, TYPE, TYPE_STRING, EMPLOYEE_ID, RESOURCE_ID, 
                                CLOCK_IN, CLOCK_OUT, HOURS_WORKED, INDIRECT_CODE, INDIRECT_ID, WORK_LOCATION, WORK_TIME)
                            SELECT WORKORDER_TYPE, WORKORDER_BASE_ID, WORKORDER_LOT_ID, WORKORDER_SPLIT_ID, WORKORDER_SUB_ID, OPERATION_SEQ_NO, TYPE, TYPE_STRING, EMPLOYEE_ID, RESOURCE_ID, 
                                CLOCK_IN, CLOCK_OUT, HOURS_WORKED, INDIRECT_CODE, INDIRECT_ID, WORK_LOCATION, WORK_TIME
                                FROM UNI_LABOR_TICKET WHERE TRANSACTION_ID = {TRANSACTION_ID};""",

"DELETE_LABOUR_TICKET": """DELETE FROM UNI_LABOR_TICKET WHERE APPROVED = 0 AND VISUAL_LAB_TRANS_ID IS NULL AND TRANSACTION_ID = {TRANSACTION_ID};""",
