...
"LABOUR_SUMMARY_VISUAL_TICKETS": """DECLARE @StartDate DATE = '{FROM_DATE}';
                                    ...
                                    SELECT EMP.ID AS [Employee ID], EMP.FIRST_NAME AS [First Name], EMP.LAST_NAME AS [Last Name], 
                                            ...
                                            0 AS [Vacation Hours], 0 AS [Vacation ($)], 0 AS [Bonus ($)], ISNULL(STAT_HOL.[Stat. Hours], 0) AS [Stat. Hours], 0 AS [Advance pay ($)], 0 AS [Adv. Hours], 0 AS [External ID], 0 AS [Salary Regular Hours], 0 AS [Salary Overtime Hours], 0AS [Salary Double Time Hours]
                                    FROM EMPLOYEE EMP
...
"LABOUR_SUMMARY_SUDO_TABLE_FILTER": """DECLARE @StartDate DATE = '{FROM_DATE}';
                                        ...
                                        WITH REG_HRS AS  (SELECT EMPLOYEE_ID, SUM(HOURS_WORKED) AS 'Regular Hours' FROM UNI_LABOR_TICKET WHERE WORK_TIME = 'Regular Time' AND HOURS_WORKED IS NOT NULL AND TRANSACTION_DATE BETWEEN @StartDate AND @EndDate AND APPROVED = 1 GROUP BY EMPLOYEE_ID),
                                            OVER_HRS AS (SELECT EMPLOYEE_ID, SUM(HOURS_WORKED) AS 'Overtime (1.5)' FROM UNI_LABOR_TICKET WHERE WORK_TIME = 'Over Time' AND HOURS_WORKED IS NOT NULL AND TRANSACTION_DATE BETWEEN @StartDate AND @EndDate AND APPROVED = 1 GROUP BY EMPLOYEE_ID),
                                            DOUB_HRS AS (SELECT EMPLOYEE_ID, SUM(HOURS_WORKED) AS 'Double Time (2)' FROM UNI_LABOR_TICKET WHERE WORK_TIME = 'Double Time' AND HOURS_WORKED IS NOT NULL AND TRANSACTION_DATE BETWEEN @StartDate AND @EndDate AND APPROVED = 1 GROUP BY EMPLOYEE_ID),
                                            STAT_HOL AS (SELECT EMPLOYEE_ID, SUM(HOURS_WORKED) AS 'Double Time (2)' FROM UNI_LABOR_TICKET WHERE WORK_TIME = 'STAT HOLIDAYS' AND HOURS_WORKED IS NOT NULL AND TRANSACTION_DATE BETWEEN @StartDate AND @EndDate AND APPROVED = 1 GROUP BY EMPLOYEE_ID)
